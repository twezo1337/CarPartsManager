...
begin
  // Отключаем связь с визуальным компонентом
 DataModule2.Q_v_goods_operations.DisableControls;
 // Деактивируем НД Q_v_goods_operations
 DataModule2.Q_v_goods_operations.active := false;
 // Очищаем SQL код в НД Q_v_goods_operations
 DataModule2.Q_v_goods_operations.SQL.Clear;
 // Добавляем базовую часть SQL запроса на выбор данных
 DataModule2.Q_v_goods_operations.SQL.Add
 ('SELECT * FROM view_goods_operations ');
 // В зависимости от выбранного элемента в RadioGroup1 добавляем
 // условие выборки в SQL запрос
 ...
 // Активируем НД Q_v_goods_operations
 DataModule2.Q_v_goods_operations.active := true;
 // Включаем связь с визуальным компонентом
